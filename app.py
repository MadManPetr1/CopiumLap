...
def get_sorted_leaderboard():
    return sorted(leaderboard.items(),
                  key=lambda x: (-x[1]['points'], -x[1]['timer'], x[0]))
...
def index():
    if 'timer_start' not in session or not isinstance(
            session.get('timer_start'), int):
    ...
    if request.method == "POST" and (("reroll" in request.form) or
                                     ("mode" in request.form)):
        if cd_main == 0:
            set_cooldown("main", 1)
            if mode == "motivation":
                msg = random.choice(
                    MOTIVATIONS) if MOTIVATIONS else "No Motivations Loaded."
                gain = random.randint(8, 12)
                set_last_gain("Motivation", gain)
            else:
                msg = random.choice(
                    INSULTS) if INSULTS else "No Insults Loaded."
    ...
    if request.method == "POST" and "excuse_reroll" in request.form:
        if cd_excuse == 0:
            ...
            excuse = random.choice(
                EXCUSES) if EXCUSES else "No Excuses Loaded."
    ...
    if "main_msg" not in session:
        msg = random.choice(
            MOTIVATIONS if mode == "motivation" else INSULTS) if (
                MOTIVATIONS or INSULTS) else "No Content Loaded."
        session["main_msg"] = msg
        add_history(f"{mode}_history", msg)
